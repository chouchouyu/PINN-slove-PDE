...
from deepbsde.BlackScholesBarenblatt import BlackScholesBarenblatt  # BSB equation definition
from deepbsde.DeepBSDE import BlackScholesBarenblattSolver, rel_error_l2  # DeepBSDE solver and error function
from fbsnn.Utils import figsize, set_seed  # Utility functions for figure sizing and random seed setting
...
plt.rcParams['font.sans-serif'] = ['Arial Unicode MS', 'Heiti TC', 'Microsoft YaHei']  # Use system-supported Chinese fonts
...
def run_fbsnn_100d():
    ...
    layers = [D + 1] + 4 * [256] + [1]  # Neural network architecture: input -> 4x256 hidden -> output
    Xi = np.array([1.0, 0.5] * int(D / 2))[None, :]  # Initial condition as numpy array with batch dimension
    T = 1.0  # Terminal time
...
def compare_methods():
    ...
    print("="*80)
    print(f"Unified parameters: dimension={D}, time T={T}, interest rate r={r}, volatility σ={sigma}")
    ...
    u_fbsnn_estimate = Y_pred[0, 0, 0] if hasattr(Y_pred, '__len__') else Y_pred  # Extract scalar estimate
    ...
    print(f"{'Metric':<20} {'DeepBSDE Standard':<14} {'DeepBSDE Legendre':<16} {'FBSNN':<12}")
    print("-" * 70)
    print(f"{'Estimate':<20} {u_deepbsde_std:<14.6f} {u_deepbsde_limits:<16.6f} {u_fbsnn_estimate:<12.6f}")
    print(f"{'Analytical':<20} {u_analytical:<14.6f} {u_analytical:<16.6f} {u_analytical:<12.6f}")
    print(f"{'Error(Analytical)%':<20} {deepbsde_std_error_analytical:<14.2f} {deepbsde_limits_error_analytical:<16.2f} {fbsnn_error_analytical:<12.2f}")
    print(f"{'Compute Time(s)':<20} {deepbsde_std_time:<14.2f} {deepbsde_limits_time:<16.2f} {fbsnn_time:<12.2f}")
    ...
    plt.axhline(y=u_analytical, color='red', linestyle='--', label=f'Analytical: {u_analytical:.2f}')  # Reference line
    ...
    plt.subplot(2, 3, 3)
    errors_analytical = [deepbsde_std_error_analytical, 
                        deepbsde_limits_error_analytical, fbsnn_error_analytical]
    bars = plt.bar(methods, errors_analytical, color=colors, alpha=0.7)
    plt.axhline(y=5, color='red', linestyle='--', label='5% Error Threshold')  # Error threshold
    ...
    plt.plot(resources, deepbsde_std_convergence, 's-', label='DeepBSDE Standard', color='blue')
    plt.plot(resources, deepbsde_limits_convergence, '^-', label='DeepBSDE Legendre', color='purple')
    ...
    categories = ['Accuracy', 'Computation Speed', 'Memory Efficiency', 
                 'High-Dim Adaptability', 'Theoretical Guarantee', 'Implementation Complexity']
    ...
    plt.plot(angles, deepbsde_limits_scores, '^-', linewidth=2, label='DeepBSDE Legendre')
    ...
    plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.0))  # Place legend outside plot
    
    plt.tight_layout()  # Adjust layout to prevent overlap
    plt.suptitle('100D BSB Equation Solving Method Comprehensive Comparison (with DeepBSDE Standard)', 
                fontsize=16, y=1.02)  # Add main title
    # Add timestamp to filename for unique identification
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(f"Figures/100D_deepbsdeVSfbsnn_Comparison_{timestamp}.png", dpi=300, bbox_inches='tight')  # Save figure with timestamp
    ...
    accuracy_rank = sorted(zip(methods, errors_analytical), key=lambda x: x[1])  # Sort by error
    ...
    print("   - DeepBSDE Standard: Simple implementation, strong theoretical guarantees, moderate complexity")
    print("   - DeepBSDE Legendre: Provides confidence intervals, stronger theoretical guarantees, slightly higher complexity")
    print("   - FBSNN: Best adaptability for high-dimensional problems, solid PDE framework, but longer training time")
