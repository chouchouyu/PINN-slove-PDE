...
def compare_methods():
    ...
    benchmark_price = (
        result_benchmark.us
        if hasattr(result_benchmark.us, "__len__")
        else result_benchmark.us
    )
    ...
    # 计算相对误差（相对于基准）
    deepbsde_std_error_benchmark = (
        abs(u_deepbsde_std - benchmark_price) / benchmark_price * 100
    )
    deepbsde_limits_error_benchmark = (
        abs(u_deepbsde_limits - benchmark_price) / benchmark_price * 100
    )
    fbsnn_error_benchmark = (
        abs(u_fbsnn_estimate - benchmark_price) / benchmark_price * 100
    )
    ...
    benchmark_error_analytical = (
        abs(benchmark_price - u_analytical) / u_analytical * 100
    )

    print(
        f"{'指标':<20} {'DeepBSDE基准':<14} {'DeepBSDE标准':<14} {'DeepBSDELegendre':<16} {'FBSNN':<12}"
    )
    print("-" * 80)
    print(
        f"{'估计值':<20} {benchmark_price:<14.6f} {u_deepbsde_std:<14.6f} {u_deepbsde_limits:<16.6f} {u_fbsnn_estimate:<12.6f}"
    )
    print(
        f"{'解析解':<20} {u_analytical:<14.6f} {u_analytical:<14.6f} {u_analytical:<16.6f} {u_analytical:<12.6f}"
    )
    print(
        f"{'误差(基准)%':<20} {'基准':<14} {deepbsde_std_error_benchmark:<14.2f} {deepbsde_limits_error_benchmark:<16.2f} {fbsnn_error_benchmark:<12.2f}"
    )
    print(
        f"{'误差(解析解)%':<20} {benchmark_error_analytical:<14.2f} {deepbsde_std_error_analytical:<14.2f} {deepbsde_limits_error_analytical:<16.2f} {fbsnn_error_analytical:<12.2f}"
    )
    print(
        f"{'计算时间(秒)':<20} {benchmark_time:<14.2f} {deepbsde_time:<14.2f} {deepbsde_time:<16.2f} {fbsnn_time:<12.2f}"
    )
    ...
    plt.axhline(
        y=u_analytical, color="red", linestyle="--", label=f"解析解: {u_analytical:.2f}"
    )
    plt.ylabel("解的值")
    plt.title("价格估计对比")
    plt.legend()
    ...
    plt.plot(
        resources, benchmark_convergence, "o-", label="DeepBSDE基准", color="green"
    )
    plt.plot(
        resources, deepbsde_std_convergence, "s-", label="DeepBSDE标准", color="blue"
    )
    plt.plot(
        resources,
        deepbsde_limits_convergence,
        "^-",
        label="DeepBSDELegendre",
        color="purple",
    )
    ...
    plt.plot(
        angles, deepbsde_limits_scores, "^-", linewidth=2, label="DeepBSDELegendre"
    )
