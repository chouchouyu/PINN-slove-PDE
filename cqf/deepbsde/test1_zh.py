...
def plot_improved_confidence_intervals(metrics, analytical_value, ax):
    ...
    if not metrics.get("limits_intervals"):
        ax.text(
            0.5, 0.5, "无置信区间数据", transform=ax.transAxes, ha="center", va="center"
        )
    ...
    # 使用不同的颜色标记违规点
    colors = [
        "red" if (u0 < low or u0 > high) else "blue"
    ...
    for i, ((low, high), u0, color) in enumerate(
        zip(metrics["limits_intervals"], metrics["limits_u0"], colors)
    ):
        if color == "blue":
            normal_x.append(i)
            normal_y.append(u0)
    ...
    coverage = (
        sum(
            1
            for (low, high), u0 in zip(
                metrics["limits_intervals"], metrics["limits_u0"]
            )
            if low <= u0 <= high
        )
        / len(metrics["limits_intervals"])
        * 100
    )
...
def plot_comprehensive_comparison(metrics, results_std, results_limits, d):
    ...
    # 1. 误差分布对比
    axes[0, 0].boxplot(
        [metrics["std_errors"], metrics["limits_errors"]],
        labels=["标准方法", "对偶方法"],
    )
    ...
    # 2. 训练时间对比
    axes[0, 1].boxplot(
        [metrics["std_times"], metrics["limits_times"]], labels=["标准方法", "对偶方法"]
    )
    ...
    analytical_value = (
        results_std[0][0]
        .analytical_solution(results_std[0][0].x0, results_std[0][0].tspan[0])
        .item()
    )
    axes[0, 2].axhline(
        y=analytical_value,
...
def comprehensive_comparison(d=30, num_trials=3):
    ...
    if performance_metrics["limits_intervals"]:
        analytical_value = (
            results_limits[0][0]
            .analytical_solution(results_limits[0][0].x0, results_limits[0][0].tspan[0])
            .item()
        )
    ...
    # 统计显著性检验
    if (
        len(performance_metrics["std_errors"]) > 1
        and len(performance_metrics["limits_errors"]) > 1
    ):
        t_stat, p_value = stats.ttest_ind(
            performance_metrics["std_errors"], performance_metrics["limits_errors"]
        )
    else:
    ...
    comparison_data.append(
        [
            ...
            (
                "✓ 标准方法更优"
                if std_error_mean < limits_error_mean
                else "✓ 对偶方法更优"
            ),
        ]
    )
    ...
    if performance_metrics["limits_intervals"]:
        comparison_data.append(
            [
                ...
                (
                    ...
                    else "○ 一般" if coverage_rate >= 0.7 else "✗ 较差"
                ),
            ]
        )
        ...
        comparison_data.append(
            [
                ...
                (
                    "✓ 较窄"
                    if avg_interval_width < 1.0
                    else "○ 适中" if avg_interval_width < 3.0 else "✗ 较宽"
                ),
            ]
        )
    ...
    characteristics = {
        "准确性": [
            max(0, 10 - std_error_mean * 100),
            max(0, 10 - limits_error_mean * 100),
        ],
        "计算效率": [
            max(0, 10 - std_time_mean / 10),
            max(0, 10 - limits_time_mean / 10),
        ],
    ...
    for char, scores in characteristics.items():
        ...
        recommendation = (
            "标准方法"
            if std_score > limits_score
            else "对偶方法" if limits_score > std_score else "相当"
        )
        print(
            f"{char:<15} {std_score:<10.1f} {limits_score:<10.1f} {recommendation:<10}"
        )
...
def main():
    ...
    if metrics.get("limits_intervals"):
        ...
        coverage = sum(
            1
            for interval in metrics["limits_intervals"]
            if interval[0] <= analytical_value <= interval[1]
        )
        print(
            f"   • 对偶方法提供置信区间，覆盖率为{coverage/len(metrics['limits_intervals'])*100:.1f}%"
        )
