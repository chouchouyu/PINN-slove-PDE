...
#TODO 更精确的结果，可以增加迭代次数到20000+
# TODO 优化器改进：使用AdamW优化器并添加学习率调度器

# 添加路径
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '.')))
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from fbsnn.Utils import set_seed
...
class BlackScholesBarenblattOptunaOptimizer:
    ...
    def objective(self, trial):
        ...
        try:
            ...
            n_iter1 = trial.suggest_int("n_iter1", 500, 20000, step=500)
            n_iter2 = trial.suggest_int("n_iter2", 1000, 5000, step=1000)
            ...
            # 第一阶段训练（粗调）
            print(f"试验 {trial.number}: 第一阶段训练...")
            graph1 = model.train(n_iter1, learning_rate1)
            
            # 第二阶段训练（精调）
            print(f"试验 {trial.number}: 第二阶段训练...")
            ...
            # 计算相对误差
            relative_error = np.mean(np.abs(Y_pred[:, -1, 0] - Y_analytical[:, -1, 0]) / 
            ...
            trial.set_user_attr("n_iter_total", n_iter1 + n_iter2)
    ...
    def optimize(self, n_trials=50, timeout=7200, study_name="black_scholes_barenblatt_optuna"):
        """执行超参数优化"""
        print("=" * 80)
    ...
    def visualize_results(self, save_plots=True):
        ...
        try:
            ...
            if len(completed_trials) == 1:
                fig1.update_layout(
                    xaxis=dict(range=[-0.5, 0.5])  # 设置合适的x轴范围
                )
    ...
    def _create_basic_visualizations(self, results_dir, timestamp):
        ...
        plt.savefig(f"{results_dir}/optimization_history_{timestamp}.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_model_performance(self, results_dir, timestamp):
        ...
        for i in range(n_samples_show):
            ...
            ax3.plot(t, y_true_sample, 'r--', alpha=0.7, label='解析解' if i == 0 else "")
        ax3.set_xlabel('时间 t')
        ...
        plt.tight_layout()
        plt.savefig(f"{results_dir}/model_performance_{timestamp}.png", dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def generate_report(self):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  训练时间: {self.best_trial.user_attrs['training_time']:.2f}秒\n\n")
            ...
            if completed_trials:
                errors = [t.user_attrs.get('relative_error', float('inf')) for t in completed_trials]
