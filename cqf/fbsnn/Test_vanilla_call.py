...
from cqf.fbsnn.CallOption import CallOption
...
def black_scholes_call(S, K, T, r, sigma, q=0):
    ...
    call_price = (S * np.exp(-q * T) * normal.cdf(d1)) - (
        K * np.exp(-r * T) * normal.cdf(d2)
    )
    delta = normal.cdf(d1)
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if time_to_maturity > 0:
                option_prices[i, j], deltas[i, j] = black_scholes_call(
                    S, K, time_to_maturity, r, sigma, q
                )
            else:
