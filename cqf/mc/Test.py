...
class Test(unittest.TestCase):
    ...
    def test_get_discounted_cashflow_at_t0(self):
        np.random.seed(444)
        random_walk = Paths_generater(
            3, 3, np.ones(1), 0.03, np.ones(1), np.zeros(1), np.eye(1)
        )

        def test_payoff(*l):
            return max(3 - np.sum(l), 0)

        opt = American_Option(random_walk, test_payoff)
        cashflow_matrix = np.array([[0, 0, 0, 0], [0, 0, 1, 0], [0, 2, 0, 0]])
        discount = opt._get_discounted_cashflow_at_t0(
            cashflow_matrix
        )  # 0.9608852002270883
        # 使用文档3的期望值计算表达式
    ...
    def test_get_discounted_cashflow(self):
        ...
        print(
            "-----", sum(abs(discounted - np.array([2.9113366, 0, 1.94089107])))
        )  # 3.548508153983221e-09
        ...
        print(
            "-----", sum(abs(discounted2 - np.array([2.8252936, 0, 1.82786237])))
        )  # 1.2952021677392622e-09
    ...
    def test_price1d(self):
        ...
        random_walk = Paths_generater(
            1, 300, init_price_vec, ir, vol_vec, dividend_vec, corr_mat
        )

        def test_payoff(*l):
            return max(strike - np.sum(l), 0)

        opt1 = American_Option(random_walk, test_payoff)
    ...
    def test_price100d(self):
        ...
        random_walk = Paths_generater(
            1, 50, init_price_vec, ir, vol_vec, dividend_vec, corr_mat
        )
