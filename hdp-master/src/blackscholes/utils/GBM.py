...
class GBM:
    ...
    def simulate(self, M):
        ...
        """
        simulations = []
        drift_vec = np.array([self.ir]*len(self.dividend_vec)) - np.array(self.dividend_vec)
        vol_vec = np.array(self.vol_vec)
        ...
        for _ in range(M):
            ...
            for i in range(1, self.N+1):
                ...
                sim[:, i] = np.multiply(1 + self.dt*drift_vec, sim[:, i-1]) + np.multiply(np.multiply(vol_vec, sim[:, i-1]), dW)
    ...
    def simulateV2(self, M):
        ...
        for _ in range(M):
            ...
            for i in range(1, self.N+1):
                dW = L.dot(np.random.normal(size=self.asset_num))*sqrt(self.dt)
                rand_term = np.multiply(vol_vec, dW)
                sim[:, i] = np.multiply(sim[:, i-1], np.exp((drift_vec-vol_vec**2/2)*self.dt + rand_term))
            simulations.append(sim)
        return np.array(simulations)
    ...
    def simulateV2_T(self, M):
        ...
        drift_vec = np.array([self.ir]*len(self.dividend_vec)) - np.array(self.dividend_vec)
        vol_vec = np.array(self.vol_vec)
        fixed = np.multiply(self.init_price_vec, np.exp((drift_vec-vol_vec**2/2)*self.T))
        L = np.linalg.cholesky(self.corr_mat)
        for _ in range(M):
